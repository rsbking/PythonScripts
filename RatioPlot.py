...
def multiRatio(hist1,hist2,log=False,xTitle="",yTitle="",drawPlot1Opt="",drawPlot2Opt="",norm=False,ratioMin=0.5,ratioMax=1.5):
    ...
    maxList = []
    if not norm:
        hist2[0].Draw(drawPlot2Opt)
        maxList.append(hist2[0].GetMaximum())
        for h in hist2[1:]:
        ...
        maxList.append(hist1.GetMaximum())
    ymax = max(*maxList)
    hist2[0].GetYaxis().SetRangeUser(0,ymax*1.2)
    c.cd()
    c.cd(2)
    ratios = []
    ...
    ratioHist.SetMarkerColor(hist2[0].GetLineColor())
    ratioHist.SetMarkerStyle(hist2[0].GetMarkerStyle())
    ratioHist.Draw(drawPlot1Opt)
    ...
    for h in hist2[1:]:
        ...
        ratioHist.SetMarkerStyle(h.GetMarkerStyle())
        ratioHist.SetMarkerColor(h.GetMarkerStyle())
